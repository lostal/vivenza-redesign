...
export default function ExhibitionAccordion({ data }: ExhibitionAccordionProps) {
  ...
  return (
    <Accordion type="single" collapsible className="w-full max-w-3xl mx-auto space-y-3 sm:space-y-4">
      {data.map((community) => (
        <AccordionItem value={community.id} key={community.id} className="border bg-card rounded-lg shadow-md">
          ...
          <AccordionContent className="px-4 sm:px-6 pb-4 sm:pb-6 pt-2">
            {community.showrooms.map((showroom) => (
              <div key={showroom.id} className="mb-5 sm:mb-6 p-3 sm:p-4 border border-border/70 rounded-md bg-background/50 last:mb-0">
                <h4 className="font-semibold text-sm sm:text-md text-foreground mb-2">{showroom.name}</h4>
